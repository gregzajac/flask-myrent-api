...
def token_landlord_required(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if token is None:
            abort(
                401,
                description="Missing landlord token. Please login or register as landlord.",
            )
        ...
        try:
            ...
            abort(
                401,
                description="Expired token. Please login as landlord to get new token.",
            )
        except jwt.InvalidTokenError:
            abort(
                401, description="Invalid token. Please login or register as landlord."
            )
...
def delete_file_from_s3(
    bucket_name: str, file_name: str, aws_access_key_id: str, aws_secret_access_key: str
) -> bool:
    result = False
