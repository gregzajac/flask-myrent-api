...
def test_register_landlord_missing_data(client, data, missing_field):
    response = client.post("/api/v1/landlords/register", json=data)
    response_data = response.get_json()
...
def test_register_landlord_already_used_email(client, landlord):
    ...
    assert "token" not in response_data
    assert (
        f'Landlord with identifier {landlord["identifier"]} already exists'
    ...
    assert "token" not in response_data
    assert (
        f'Landlord with email {landlord["email"]} already exists'
        in response_data["message"]
    )
...
def test_get_current_landlord(client, landlord, landlord_token):
    response = client.get(
        "/api/v1/landlords/me", headers={"Authorization": f"Bearer {landlord_token}"}
    )
    response_data = response.get_json()
...
def test_get_landlords_with_params(client, sample_data):
    response = client.get(
        "/api/v1/landlords?fields=id,identifier,first_name&sort=-id&id[lte]=2"
    )
    response_data = response.get_json()
    ...
    assert response_data["data"] == [
        {"first_name": "Andrzej", "id": 2, "identifier": "landlord2"},
        {"first_name": "Jan", "id": 1, "identifier": "landlord1"},
    ]
