...
def test_get_landlord_tenants_no_records_tenant_registered(client, tenant_token):
    response = client.get(
        "/api/v1/tenants", headers={"Authorization": f"Bearer {tenant_token}"}
    )
    response_data = response.get_json()
    expected_result = {"success": False, "message": "Only landlord functionality"}
...
def test_get_landlord_tenants_no_records_landlord_registered(client, landlord_token):
    response = client.get(
        "/api/v1/tenants", headers={"Authorization": f"Bearer {landlord_token}"}
    )
    response_data = response.get_json()
    expected_result = {"success": True, "data": [], "number_of_records": 0}
...
def test_get_landlord_tenants_with_data(client, sample_data):
    response = client.post(
        "api/v1/landlords/login", json={"identifier": "landlord3", "password": "haslo3"}
    )
...
def test_get_landlord_tenant(client, sample_data):
    ...
    response = client.get(
        "/api/v1/tenants/6", headers={"Authorization": f"Bearer {token}"}
    )
    response_data = response.get_json()
...
def test_login_tenant_wrong_identifier(client, tenant, tenant_token):
    response = client.post(
        "/api/v1/tenants/login",
        json={"identifier": "wrongidentifier", "password": tenant["password"]},
        headers={"Authorization": f"Bearer {tenant_token}"},
    )
    response_data = response.get_json()
...
def test_login_tenant_wrong_password(client, tenant, tenant_token):
    response = client.post(
        "/api/v1/tenants/login",
        json={"identifier": tenant["identifier"], "password": "wrongpassword"},
        headers={"Authorization": f"Bearer {tenant_token}"},
    )
    response_data = response.get_json()
...
def test_get_current_tenant(client, tenant, tenant_token):
    response = client.get(
        "/api/v1/tenants/me", headers={"Authorization": f"Bearer {tenant_token}"}
    )
    response_data = response.get_json()
...
def test_update_tenant_data_missing_data(
    client, tenant, tenant_token, data, missing_field
):
    response = client.put(
        f"/api/v1/tenants/1/data",
        json=data,
    ...
    assert response_data["message"][missing_field] == [
...
def test_delete_tenant_by_landlord(client, tenant, landlord_token):
    response = client.delete(
        f"/api/v1/tenants/1", headers={"Authorization": f"Bearer {landlord_token}"}
    )
    response_data = response.get_json()
...
def test_delete_tenant_by_tenant(client, tenant, tenant_token):
    response = client.delete(
        f"/api/v1/tenants/1", headers={"Authorization": f"Bearer {tenant_token}"}
    )
    response_data = response.get_json()
