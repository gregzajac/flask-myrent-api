...
def test_get_all_flats_with_params(client, sample_data):
    response = client.get(
        "/api/v1/flats?fields=id,identifier&sort=-id&id[gte]=2&limit=2&page=2"
    )
    response_data = response.get_json()
    ...
    assert response_data["data"] == [
        {"id": 4, "identifier": "Chopina 5"},
        {"id": 3, "identifier": "Tołstoja 3"},
    ]
...
def test_get_one_flat(client, sample_data):
    ...
    assert (
        ...
        == "Mieszkanie dwupokojowe z aneksem kuchennym na trzecim piętrze."
    )
...
def test_create_flat_missing_data(client, landlord_token, data, missing_field):
    response = client.post(
        "/api/v1/flats",
        json=data,
        headers={"Authorization": f"Bearer {landlord_token}"},
...
def test_update_flat_missing_data(
    client, landlord_token, flat_data, data, missing_field
):
    response = client.post(
        "/api/v1/flats",
        json=flat_data,
...
def test_update_flat_existing_identifier(
    client, landlord_token, flat_data, flat_2_data
):
    response = client.post(
        "/api/v1/flats",
        json=flat_data,
...
def test_delete_flat(client, landlord_token, flat_data):
    ...
    response = client.delete(
        "/api/v1/flats/1", headers={"Authorization": f"Bearer {landlord_token}"}
    )
    response_data = response.get_json()
